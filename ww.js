...
function guessWord () {
    ...
    if(validGuess == true && gameOver == false && guess[4]!=undefined){
        ...
        if(letters[0] == 2 && letters[1] == 2 && letters[2] == 2 && letters[3] == 2 && letters[4] == 2){
            ...
            showScore(score, guesses, streak);
...
function scoreFill() {
    ...
    while (counting) {
        if (leaderBoard[listLength][randomNum] != null) {
            listLength++;
        }
        else {
            counting = false;
    ...
    for (let i = 0; i < listLength; i++) {
        if (leaderBoard[i + 1][randomNum] == score) {
            checkr = false;
        }
    }
    if (checkr) {
        leaderBoard[listLength][randomNum] = score;
...
function showScore(score, guesses){
    ...
    //display streak
    let showStreak = document.createElement("h2");
    showStreak.innerHTML = "Streak: "+ streak;
    showStreak.className += "description";
    document.querySelector(".introText").appendChild(showStreak);
}

function showWord(result){
    //display lost message
    let lostMessage = document.createElement("h1");
    lostMessage.className += "largeHeader";
    lostMessage.innerHTML = "Sorry You Lost!";
    document.querySelector(".introText").appendChild(lostMessage);

    //display correct word
    let wordElement = document.createElement("h2");
    wordElement.className += "description";
    wordElement.innerHTML = "Correct word: " + result;
