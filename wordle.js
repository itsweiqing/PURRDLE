...
let streak = 0; // number of times in a row a user has guesses a word
...
function guessWord () {
    ...
    if(validGuess == true && gameOver == false && guess[4]!=undefined){
        ...
        if(letters[0] == 2 && letters[1] == 2 && letters[2] == 2 && letters[3] == 2 && letters[4] == 2){
            ...
            showScore(score, guesses);
            console.log("amount of guesses:"+guesses);
            // holds game board and guess box
            var game = document.getElementById("mainGame");
            //hide game board when displaying leaderboard
        ...
        //Does the scoring function (takes guesses)
        score = scoring(guesses);
...
//the scoring function takes in the amount of guesses and does scoring stuff with stuff (just a bunch of stuff ig)
function scoring(tries) {

    //this counts the amount of tries it took to get a word, if the user got the word on the fourth row wrdTry would have a value of 4
    ...
    if (tries < 6) {
        ...
        return (1 - ((tries / 6) / 2))*100;
    }
    //loose state (score is already set to 0)
    else {
        streak = 0;
        return 0;
    }
...
function boardSort(score) {
    let leaderBoard;
    let temp, listLength, counting = true;
    //finds limit for the leaderboard
    while (counting) {
        if (leaderBoard[listLength] != null) {
            listLength++;
        }
        else {
            counting = false;
    ...
    //adds the score to the board
    leaderBoard[listLength + 1] = score;
    listLength++;
...
function showScore(score, guesses){
    // display calculated score 
    let scoreElement = document.createElement("h1");
    scoreElement.className += "largeHeader";
    scoreElement.innerHTML = "Your Score: "+score;
    document.querySelector(".introText").appendChild(scoreElement);
    // display number of guesses
    let showGuesses = document.createElement("h2");
    showGuesses.innerHTML = "Number of guesses: "+ guesses;
    showGuesses.className += "description";
    document.querySelector(".introText").appendChild(showGuesses);  
}
function showWord(result){
    let wordElement = document.createElement("h1");
    wordElement.className += "largeHeader";
    scoreElement.innerHTML = "Correct word:" + result;
    document.querySelector(".introText").appendChild(wordElement);
